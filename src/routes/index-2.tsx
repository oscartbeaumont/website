...
// TODO: Assignment of the text starting seems inconsistent
// TODO: Easter egg when clicking my name too many times.
function Header() {
  return (
    <header class="mb-16">
            ...
            <div class="relative group">
              ...
              <div class="relative bg-white border border-gray-100 rounded-lg p-8 group-hover:border-gray-200 transition-all duration-200 shadow-sm group-hover:shadow-md">
                <div class="flex items-start justify-between mb-4">
                  <h3 class="text-2xl font-bold text-gray-900 tracking-tight">{project.name}</h3>
                  <div class="flex items-center gap-2 text-gray-600">
                    <StarIcon />
                    <span class="font-medium">{project.stars}</span>
                  </div>
                </div>
                <p class="text-xl text-gray-600 mb-6 leading-relaxed font-medium">{project.description}</p>
                <div class="flex flex-wrap gap-3 mb-6">
                  {project.tech.map((tech: string) => (
                    <span class="px-4 py-2 bg-gray-100 text-gray-700 rounded-full font-medium hover:bg-gray-200 transition-colors duration-200">
                      {tech}
                    </span>
                  ))}
                </div>
                <a
                  ...
                  class="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 font-medium text-lg transition-colors duration-200"
                >
                  View Project
                  <ExternalLinkIcon />
                </a>
  ...
  return (
    <section class="mb-16">
      <div class="text-center mb-12">
        <h2 class="text-5xl md:text-6xl font-bold mb-4 tracking-tight text-gray-900">Skills & Technologies</h2>
        <p class="text-xl text-gray-600 font-medium leading-relaxed">
          Building with modern tools and <span class="text-lg font-light">cutting-edge technologies</span>
        </p>
      </div>
      <div class="grid md:grid-cols-2 gap-12">
        {skillCategories.map((category) => (
          <div class="relative group">
            <div class="absolute inset-0 bg-gradient-to-br from-gray-50 to-blue-50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            <div class="relative bg-white border border-gray-100 rounded-lg p-8 group-hover:border-gray-200 transition-all duration-200 shadow-sm group-hover:shadow-md">
              <h3 class="text-2xl font-bold mb-6 text-gray-900 tracking-tight">{category.title}</h3>
              <div class="flex flex-wrap gap-3">
                {category.skills.map((skill) => (
                  <span class="px-4 py-2 bg-gray-100 border border-gray-200 text-gray-700 rounded-full font-medium hover:bg-gray-200 hover:border-gray-300 transition-all duration-200 cursor-default">
                    {skill}
                  </span>
                ))}
              </div>
